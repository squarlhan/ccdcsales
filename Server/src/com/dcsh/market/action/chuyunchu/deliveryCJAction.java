...
public class deliveryCJAction implements Preparable{
	...
	public deliveryCJAction(WareHouseService service) {
	...
	public String execute() throws Exception {	
    	...
    	for(int i=0;i<=this.getProduct().size()-1;i++){
    	    ...
    	    this.chukumx = 
    		    new Chukumx(newproduct,null,newspecification,"zzzzzzzzz",this.getNumber().get(i));
    	...
    	List<CankuPriv> user = (List<CankuPriv>)session.get("tempuser");
    	  ...
    	  this.chuku = new Chuku(
    			service.getCangkuById(cankuaim),
          		service.getCangkuByType(7).get(0),
          		user.get(0).getUser(),
          		null,
          		this.getBno().trim(),
          		new Date(),
	...
	public String print() throws Exception{
		...
		for(int i=0;i<=this.getProduct().size()-1;i++){
			...
			this.epi = 
	    		new EntryPrintInfo(product.get(0).getName(),specification.get(0).getName(),specification.get(0).getPackType(),this.getNumber().get(i),this.getSumweight().get(i).toString(),"zzzzzzzzz","");
			resultList.add(epi);
		}
		return "print";
	}
