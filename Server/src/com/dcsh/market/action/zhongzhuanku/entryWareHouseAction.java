...
public class entryWareHouseAction implements Preparable{
	...
	public void setSaletype(List<Integer> saletype) {
	...
	public entryWareHouseAction(ZhongZhuanKuService service) {
	...
	public String execute() throws Exception{
		...
		for(int i=0;i<=this.getProduct().size()-1;i++){
			...
			this.rkmx = 
	    		new Rkmx(null,newproduct,newspecification,this.getPch().get(i).trim(),this.getNumber().get(i),this.getSaletype().get(i).byteValue(),(byte)1,this.getMemo().get(i).trim());
			temprumxs.add(rkmx);
		}
