...
function setnumber(obj,line)
{
	...
	function insertRecord(){
		...
		select3.setAttribute("id","saletype["+count+"]");
		select3.setAttribute("name","saletype["+count+"]");
		...
		var orginsts=document.getElementById("saletype[0]")
		 ...
		 for(var k=0;k<orginsts.children.length;k++){
	          var optss=document.createElement("option");
	          optss.value=orginsts.children[k].value;
	          optss.text=orginsts.children[k].text;
	          select3.options.add(optss);
	          }
	...
	function checkRKNull()
	{
		...
		if(confirm("确认入库操作？")){
			document.myform.action = "zzkentrywarehouse.action";
			return true;
		}
		else
			return false;
	...
	function checkPrintNull()
	{
		...
		if(confirm("确认打印操作？")){
			document.myform.action = "zzkentrywarehouse!print.action";
			return true;
		}
		else
			return false;
...
<h2>中转库产成品入库单</h2>
  ...
  <table class="list_table"  align="center" width="100%" id="tb0">
     ...
     <tr  align="center" bgcolor="<s:if test="#index.odd == true">#ffffff</s:if><s:else>#EDEDED</s:else>">
       ...
       <td><s:select id="saletype[0]" name="saletype[0]" list="#{'1':'内销','2':'定向','3':'外销'}" key="key" value="value"/></td>
