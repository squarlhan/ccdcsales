...
function setnumber(obj,line)
{
	...
	function checkNull()
	{
		...
		if(confirm("确认装车？")){
			document.myform.action = "cycdeliverycj.action";
			return true;
		}
		else
			return false;
	...
	//下面开始分页
	 ...
	 var perpage = 10;
	 var cankunames = new Array(
			    <s:iterator id="result" value="resultList">
				    "<s:property value='#result.cankuName'/>",
			    </s:iterator>
		    0); 
	var pchs = new Array(
		    <s:iterator id="result" value="resultList">
...
<h3>储运处对应工厂库可用产品明细</h3>
  ...
  <tr>
           <td><s:text name="请选择车间"></s:text>
  	       </td>
  	       <td><s:select name="cankuaim" list="cankusList" label="请选择车间"
