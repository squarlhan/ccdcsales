...
public interface ZhuChangKuService {
	 ...
	 * @return 用户或者空
	 */
	List<CankuPriv> loginZhuChangKu(String username,byte[] Password);
	 ...
	 * @param rkxx 生成的入库明细
	 */			
	void doentryZhuchangku(List<Chukumx> chukumxes,Rkxx rkxx);
	/**
	 * 产成品进销存（发运）日报
	 * 产品列表
	 * @param canku 仓库id
     ...
     * @param ckid 要查询的仓库id
     */  
    List<Kcxx> listZhuChangKuAll(int ckid);
    /**
     * 查报表信息
     * @param ckid 要查询的仓库id
     * @param date  要查询的日期
     */  
    List<ReportPmx> listZhuChangKuOther(int ckid,Date date);
/**
 * 获得仓库信息
 * @param cangkuId 仓库ID
 * @return 仓库对象
